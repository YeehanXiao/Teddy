...
After transcript-level quantification, TEDDY provides `processGTF()` to extract TE-chimeric transcript features from the annotated unified GTF. This step identifies transcript exons whose genomic coordinates intersect TE annotations, and returns the corresponding transcript structures with TE name, family, and class information. The resulting features can be further characterized with annotateTEPosition() to resolve the position of individual TE loci relative to exon and transcript boundaries.
...
```{r annotate-te-position, eval=FALSE}
...
The resulting GRanges object contains the additional information describing the relative position of each TE within the exon, its association with transcript-defining boundaries, and the proportion of the exon sequence contributed by the TE.
...
- `TE_category` provides the exon-rank-based classification. `TE-initiated` indicates that the TE overlaps the first exon, `TE-terminated` the terminal exon, and `TE-exonized` an internal exon.

- `TE_position` describes where the TE-derived sequence lies within the exon. `5prime` and `3prime` indicate that the TE reaches or lies within the user-defined `boundary_tolerance` of the corresponding exon boundary, `both` indicates association with both boundaries, and `internal` indicates that the TE does not approach either boundary. A small tolerance can accommodate minor coordinate variation introduced, such as during transcript reconstruction or TE annotation.

- `TE_boundary` combines `TE_category` and `TE_position` to to indicate potential boundary-associated roles. For example, a TE located near the 5′ boundary of the first exon is annotated as `TE-associated-initiation`, whereas a TE near the 3′ boundary of an internal exon is annotated as `TE-associated-splice-donor`. These are structural association with transcript boundaries rather than direct evidence of exact regulatory or splice-site motif.

- TE positional annotations can be visualized with `formTEPositionPlot()`. See the [downstream analysis tutorial](downstream-analysis.html#visualize-detailed-te-positions-with-formtepositionplot) for examples.
