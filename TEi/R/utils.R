...
processInsertion <- function(file, max.gapwidth = 10, threads = 5) {
  bed <- tryCatch(
            expr = {
  ...
  predict <- parallel::mclapply(index, FUN = function(x, min.gapwidth) {
    location <- bed[x]
  ...
  }, min.gapwidth = max.gapwidth, mc.cores = threads)
